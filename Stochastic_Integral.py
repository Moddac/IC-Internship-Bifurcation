    ...
    def solve(self,T,N=int(1e3),PDF=True,trajectory=True,nb_simul=int(1e4),nb_trajectory=5,reference_solution=None):
        ...
        if PDF:
            ...
            if reference_solution is not None:
                ...
                Ys_T = np.array([reference_solution(T) for _ in range(nb_simul)])
...
def histPDF(data,nb_bins=30):
    ...
    a random variable
    """
    bins = np.linspace(data.min()-.1,data.max(),nb_bins)
...
if __name__=='__main__':
    ...
    """
    #-----Init-----
    #If True, shows PDF (needs a lot of simulation)
    #If False, shows the trajectories of the solutions (less simulations)
    PDF = True 
    trajectory = False
    T = 1
    N = 1000
    ...
    #     L_ = np.array(L)
    #     L_ = L_[:,-1]
    #     histogram_L, bins_L = histPDF(L_)

    #     #-----Reference if solution known-----
    def p_ref(t):
        return t*chisquare(1)-t
    #     G = np.array([p_ref(T) for _ in range(nb_simul)])
    #     histogram_G, bins_G = histPDF(G)

    #     ax.plot(bins_L,histogram_L,label='Simulation')
    #     ax.plot(bins_G,histogram_G,label='Reference')
    #     ax.legend()
    # if trajectory:
    #     fig, ax = plt.subplots()
    #     #Plots each trajectory
    #     for l in L:
    #         ax.plot(np.linspace(0,T,N+1),l)
        
    # plt.show()

    SDE.solve(T=1,reference_solution=p_ref)