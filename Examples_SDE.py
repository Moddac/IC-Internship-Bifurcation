...
if __name__=='__main__':
    ...
    # SDE_Brownian.solve(T=1,X_0=X_0,reference_solution=reference_solution_Brownian,PDF=True,trajectory=True,nb_simul=100_000)
    ...
    def p_ref_OU(t):
        return np.exp(-α*t)*X_0 + np.sqrt(2*σ)*np.exp(-α*t)*SDE_int.simul(t,N=1000,X_0=0)[-1]
    def traj_ref_OU(t):
        return np.exp(-α*t)*X_0
    
    # reference_solution_OU = {"proba": p_ref_OU, "traj": traj_ref_OU}
    # SDE_OU.solve(T=1,X_0=X_0,reference_solution=reference_solution_OU,PDF=True,trajectory=True,nb_simul=100_000)
    ...
    SDE_GBM.solve(T=1,X_0=X_0,reference_solution=reference_solution_GBM,PDF=True,trajectory=True,nb_simul=100_000)