...
parser.add_argument("-n", "--name", help="Name of the json file. Parameters are replaced by their values in default name.", default="Data_MC_noise_N_Np_dt_theta_gamma_epsilon.json")
...
def SDEsolve(N, N_p, dt, θ, γ, ε, β, X_0, N_space, noise):
    ...
    for n in range(N+1):
        ...
        if γ==0.:
            ...
            q_old = q
            q += γ*Y*dt 
            Y += γ*µ(q_old)*dt - γ**2*Y*dt + γ*σ(Y)*η*dt
...
def bifurcation_scheme(file_path, N, N_p, dt, θ, γ, ε, βs, noise):
    ...
    # -----Init-----
    N_points = 10
    N_space = np.linspace(0, N, N_points, dtype=int)
    βs = checkFile(file_path, N, N_p, dt, θ, γ, ε, βs)
...
if __name__ == '__main__':
    ...
    if args.name == "Data_MC_noise_N_Np_dt_theta_gamma_epsilon.json":
        if args.noise=="white":
            FILE_NAME = f"Data___MC___{args.noise}___N{args.N:_}___Np{args.N_p:_}___dt{args.dt:_}___thet{args.theta:_}___gam{args.gamma:_}.json"
        if args.noise=="OU":
            FILE_NAME = f"Data___MC___{args.noise}___N{args.N:_}___Np{args.N_p:_}___dt{args.dt:_}___thet{args.theta:_}___gam{args.gamma:_}___eps{args.epsilon:_}.json"
    else:
        FILE_NAME = args.name
