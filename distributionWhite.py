...
def ρ_st(x, θ, m, α, _σ, σ_m):
    if f'{θ},{m},{α},{_σ},{σ_m}' not in Zs.keys():
        Zs[f'{θ},{m},{α},{_σ},{σ_m}'] = integrate.quad(
            lambda y: np.exp(-f_m_det(y, m, α, θ, _σ, σ_m)), -np.inf, np.inf)[0]
    Z = Zs[f'{θ},{m},{α},{_σ},{σ_m}']
    return np.exp(-f_m_det(x, m, α, θ, _σ, σ_m))/Z
...
def σ_c(θ, α, σ_m):
    ...
    return optimize.root_scalar(f, bracket=[0.1, 100.]).root
