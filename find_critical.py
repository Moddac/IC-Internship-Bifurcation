"""
Find the critical value of sigma  
"""
import numpy as np
...
def R_prime(θ,m,α,_σ,σ_m):
    mean = integrate.quad(lambda x: x*np.arctan(σ_m*x/_σ)*ρ_st(x,θ,m,α,_σ,σ_m), -np.inf, np.inf)[0]
    return 2*θ/(_σ*σ_m)*mean

def σ_c(θ,α,σ_m):
    f = lambda _σ: R_prime(θ,0,α,_σ,σ_m) - 1
